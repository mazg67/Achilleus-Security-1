...
const COLS = [
  ...
  { key: "pin", label: "PIN", width: "18%" },
  { key: "host", label: "Host", width: "14%" },
  { key: "arrival", label: "Arrival", width: "12%" },
] as const;

export function SecurityListDocument({
  detail,
  generatedAt,
  origin,
}: {
  ...
  return (
    <Document title={`Hospitality Security List - vs ${fixture.opponent}`}>
      <Page size="A4" style={sharedStyles.page}>
        ...
        <PdfFooter generatedAt={generatedAt} contactNote={SECURITY_CONTACT_NOTE} />
      </Page>
    </Document>
  );
}
